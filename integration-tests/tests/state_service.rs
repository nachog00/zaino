...
mod get {
    ...
    use zebra_rpc::methods::AddressStrings;
    ...
    pub(crate) async fn state_service_get_address_balance(validator: &str) {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
    ...
    pub(crate) async fn state_service_get_address_balance_testnet() {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
    ...
    pub(crate) async fn state_service_get_block_raw(
        validator: &str,
        chain_cache: Option<std::path::PathBuf>,
        network: services::network::Network,
    ) {
    ...
    pub(crate) async fn state_service_get_block_object(
        validator: &str,
        chain_cache: Option<std::path::PathBuf>,
        network: services::network::Network,
    ) {
    ...
    pub(crate) async fn state_service_get_raw_mempool(validator: &str) {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(validator, None, true, true, None).await;
        let clients = test_manager
            .clients
    ...
    pub(crate) async fn state_service_get_raw_mempool_testnet() {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(
                "zebrad",
                ZEBRAD_TESTNET_CACHE_DIR.clone(),
    ...
    pub(crate) async fn state_service_z_get_treestate(validator: &str) {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(validator, None, true, true, None).await;
    ...
    pub(crate) async fn state_service_z_get_treestate_testnet() {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(
                "zebrad",
                ZEBRAD_TESTNET_CACHE_DIR.clone(),
    ...
    pub(crate) async fn state_service_z_get_subtrees_by_index(validator: &str) {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(validator, None, true, true, None).await;
    ...
    pub(crate) async fn state_service_z_get_subtrees_by_index_testnet() {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(
                "zebrad",
                ZEBRAD_TESTNET_CACHE_DIR.clone(),
    ...
    pub(crate) async fn state_service_get_raw_transaction(validator: &str) {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(validator, None, true, true, None).await;
    ...
    pub(crate) async fn state_service_get_raw_transaction_testnet() {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(
                "zebrad",
                ZEBRAD_TESTNET_CACHE_DIR.clone(),
    ...
    pub(crate) async fn state_service_get_address_tx_ids(validator: &str) {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(validator, None, true, true, None).await;
    ...
    pub(crate) async fn state_service_get_address_tx_ids_testnet() {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(
                "zebrad",
                ZEBRAD_TESTNET_CACHE_DIR.clone(),
    ...
    pub(crate) async fn state_service_get_address_utxos(validator: &str) {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(validator, None, true, true, None).await;
    ...
    pub(crate) async fn state_service_get_address_utxos_testnet() {
        let (mut test_manager, _fetch_service, fetch_service_subscriber, state_service) =
            create_test_manager_and_services(
                "zebrad",
                ZEBRAD_TESTNET_CACHE_DIR.clone(),
